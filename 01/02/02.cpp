...
std::vector<int32_t> sumVectorsWorker(const std::vector<int32_t>& vectorA,
                                    const std::vector<int32_t>& vectorB,
                                    const int32_t& chunkStartIndex,
                                    const int32_t& chunkEndIndex) {
                                    std::vector<int32_t> resultingVector{};
    resultingVector.resize(chunkEndIndex - chunkStartIndex);
    for (int32_t elementIndex = chunkStartIndex; elementIndex <= chunkEndIndex; elementIndex++) {
        resultingVector.push_back(vectorA.at(elementIndex) + vectorB.at(elementIndex));
    }
    return resultingVector;
}

std::chrono::duration<double> sumVectorsWithThreading(const std::vector<int32_t>& vectorA, const std::vector<int32_t>& vectorB, int32_t threadingFactor) {
    ...
    while (chunkStartIndex + vectorChunkSize < vectorA.size()) {
        ...
        workerThreads.push_back(std::thread(sumVectorsWorker, vectorA, vectorB, chunkStartIndex, chunkEndIndex));
        chunkStartIndex = chunkEndIndex + 1;
    }
    workerThreads.push_back(std::thread(sumVectorsWorker, vectorA, vectorB, chunkStartIndex, vectorA.size() - 1));
...
int32_t main() {
    ...
    for (const auto concurrencyFactor : concurrencyFactors) {
        ...
        for (const auto vectorToSum : vectorsToSum) {
            const auto processingTime = sumVectorsWithThreading(vectorToSum, vectorToSum, concurrencyFactor);
